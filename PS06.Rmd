...
sum(.95^2,results2)
mean((0.9025-results)^2)
```

```{r}
lm_MSE <- MSE((.95)^2-results)
lm_bias2 <- bias2(results,0.95^2)
lm_var <- var(results)

l_sum <- sum(.95^2,results)
lm_ir <- l_sum - lm_bias2 - lm_var 
l <- data.frame(Model = "LM",MSE = lm_MSE, Bias_square= lm_bias2,var = lm_var, Irreducible = lm_ir, sum = l_sum )
```

```{r}
sp_MSE <- MSE((.95)^2-results2)
sp_bias2 <- bias2(results2,0.95^2)
sp_var  <-  var(results2)

s_sum <-sum(.95^2,results2)
sp_ir <- s_sum - sp_var -sp_bias2
s <- data.frame(Model = "Spline",MSE = sp_MSE, Bias_square= sp_bias2,var = sp_var, Irreducible = sp_ir, sum = s_sum)
output <- full_join(l,s)
output %>% kable(digits=9)
```
...
3. The Spline model, as although it has a very high variance model to model, it captures the desired value far more often. 
